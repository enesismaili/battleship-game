...
// Generates layout and assigns each enemy ship a random orientation and set of coordinates; returns all placed ships
export const placeAllEnemyShips = (enemyShips) => {
    let enemyLayout = generateEmptyLayout();

    return enemyShips.map((ship) => {
        while (true) {
            let decoratedShip = randomizeShipProps(ship);

            if (canBePlaced(decoratedShip, enemyLayout)) {
                enemyLayout = putEntityInLayout(enemyLayout, decoratedShip, SQUARE_STATE.ship);
...
// Generate a random orientation and starting index on board for enemy ships
...
// Gets the neighboring squares to a successful enemy hit
